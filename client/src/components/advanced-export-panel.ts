...
import { XF_TEMPLATES, contentToExperienceFragment, experienceFragmentToJcr, experienceFragmentToHtml } from '../lib/aem-experience-fragments.js';
import { STYLE_GROUPS, generateStylePolicy, generateStyleSystemCss } from '../lib/aem-style-system.js';
import { generateComponentDialog, dialogToXml, dialogToJson, generateDesignDialog } from '../lib/aem-component-dialogs.js';
import { generateComponentClientLib, generateClientLibContentXml, generateBaseClientLib } from '../lib/aem-clientlibs.js';
...
export class AdvancedExportPanel extends LitElement {
    ...
    return html`
      <div class="panel">
        ...
        <div class="panel-body ${this.expanded ? 'expanded' : ''}">
          ${this.content || this.contents.length > 0 ? html`
            <div class="tabs">
              <button class="tab ${this.activeTab === 'package' ? 'active' : ''}" @click=${() => this.activeTab = 'package'}>
                📦 Package
              </button>
              <button class="tab ${this.activeTab === 'xf' ? 'active' : ''}" @click=${() => this.activeTab = 'xf'}>
                🧩 Experience Fragment
              </button>
              <button class="tab ${this.activeTab === 'styles' ? 'active' : ''}" @click=${() => this.activeTab = 'styles'}>
                🎨 Style System
              </button>
              <button class="tab ${this.activeTab === 'dialog' ? 'active' : ''}" @click=${() => this.activeTab = 'dialog'}>
                ⚙️ Component Dialog
              </button>
              <button class="tab ${this.activeTab === 'clientlib' ? 'active' : ''}" @click=${() => this.activeTab = 'clientlib'}>
                📁 ClientLib
              </button>
              <button class="tab ${this.activeTab === 'deploy' ? 'active' : ''}" @click=${() => this.activeTab = 'deploy'}>
                🚀 Deploy to AEM
              </button>
            </div>
            ...
            ${this.status ? html`
              <div class="status ${this.status.type}">${this.status.message}</div>
            ` : ''}
          ` : html`
            <div class="empty-state">
              Select content to access export options
            </div>
          `}
        </div>
      </div>
      ...
      <div class="section">
        ...
        <div class="grid-2">
          <div class="form-group">
            ...
            <input type="text" .value=${this.packageName} @input=${(e: Event) => this.packageName = (e.target as HTMLInputElement).value} />
          </div>
          <div class="form-group">
            <label>Group</label>
            <input type="text" .value=${this.packageGroup} @input=${(e: Event) => this.packageGroup = (e.target as HTMLInputElement).value} />
          </div>
        </div>
        <div class="form-group">
          <label>Version</label>
          <input type="text" .value=${this.packageVersion} @input=${(e: Event) => this.packageVersion = (e.target as HTMLInputElement).value} />
        </div>
      </div>
  ...
  private renderXfTab() {
    ...
    return html`
      <div class="section">
        ...
        <div class="grid-2">
          ${XF_TEMPLATES.map(t => html`
            <div class="card ${this.selectedXfTemplate === t.id ? 'selected' : ''}" @click=${() => this.selectedXfTemplate = t.id}>
      ...
      ${template ? html`
        <div class="section">
          ...
          <div class="checkbox-group">
            ${template.variations.map(v => html`
              <div
      ...
      <div class="actions">
        <button class="btn btn-primary" @click=${this.downloadXfJcr}>
          ⬇️ Download JCR JSON
        </button>
        <button class="btn btn-secondary" @click=${this.downloadXfHtml}>
          📄 Download HTML
        </button>
      </div>
    `;
  ...
  private renderStylesTab() {
    ...
    return html`
      <div class="section">
        ...
        ${styleGroups.length > 0 ? html`
          ${styleGroups.map(group => html`
            <div class="form-group">
              <label>${group.name}</label>
              <div class="checkbox-group">
                ${group.styles.map(style => html`
                  <div class="checkbox-item">
                    <code>${style.cssClasses || 'default'}</code> ${style.name}
                  </div>
                `)}
              </div>
            </div>
          `)}
        ` : html`
          <p>No style groups defined for ${componentType}</p>
        `}
      </div>
      ...
      <div class="actions">
        ...
        <button class="btn btn-secondary" @click=${() => this.downloadStyleCss(componentType)}>
          🎨 Download CSS
        </button>
      </div>
    `;
    ...
    return html`
      <div class="section">
        ...
        <div class="form-group">
          ...
          <div class="checkbox-group">
            <div class="checkbox-item ${this.dialogFormat === 'xml' ? 'selected' : ''}" @click=${() => this.dialogFormat = 'xml'}>
              XML (_cq_dialog.xml)
            </div>
            <div class="checkbox-item ${this.dialogFormat === 'json' ? 'selected' : ''}" @click=${() => this.dialogFormat = 'json'}>
              JSON (API)
            </div>
          </div>
        </div>
      ...
      ${dialog ? html`
        <div class="section">
          ...
          <div class="code-preview">
            <pre>${this.dialogFormat === 'xml' ? dialogToXml(dialog) : JSON.stringify(dialogToJson(dialog), null, 2)}</pre>
          </div>
        </div>
      ` : ''}

      <div class="actions">
        <button class="btn btn-primary" @click=${() => this.downloadDialog(componentType)}>
          ⬇️ Download Dialog
        </button>
    ...
    return html`
      <div class="section">
        ...
        <div class="form-group">
          <label>Files</label>
          <div class="checkbox-group">
            ${clientLib.files.map(f => html`
              <div class="checkbox-item">
                ${f.type === 'css' ? '🎨' : f.type === 'js' ? '📜' : '📄'} ${f.path}
              </div>
            `)}
          </div>
        </div>
      ...
      <div class="actions">
        ...
        <button class="btn btn-secondary" @click=${this.downloadBaseClientLib}>
          📦 Download Base ClientLib
        </button>
      </div>
    `;
  }
    ...
    return html`
      <div class="section">
        ...
        <div class="form-group">
          ...
          <input type="text" .value=${this.aemUrl} @input=${(e: Event) => this.aemUrl = (e.target as HTMLInputElement).value} />
        </div>
        <div class="grid-2">
          <div class="form-group">
            <label>Username</label>
            <input type="text" .value=${this.deployUsername} @input=${(e: Event) => this.deployUsername = (e.target as HTMLInputElement).value} />
          </div>
          <div class="form-group">
            <label>Password</label>
            <input type="password" .value=${this.deployPassword} @input=${(e: Event) => this.deployPassword = (e.target as HTMLInputElement).value} />
          </div>
        </div>
        <div class="form-group">
          <label>Deploy Path</label>
          <input type="text" .value=${this.deployPath} @input=${(e: Event) => this.deployPath = (e.target as HTMLInputElement).value} />
        </div>
      </div>
  ...
  private async downloadPagePackage() {
    ...
    try {
      const items = this.contents.length > 0 ? this.contents : [this.content!];
      const blob = await createPagePackage(items, {
  ...
  private downloadStylePolicy(componentType: string) {
    const policy = generateStylePolicy(componentType, `/conf/aem-component-factory/settings/wcm/policies/${componentType}`);
