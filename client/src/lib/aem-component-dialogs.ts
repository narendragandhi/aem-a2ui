...
export function dialogToXml(dialog: ComponentDialog): string {
  const tabItems = dialog.tabs.map(tab => {
    const fieldItems = tab.fields.map(field => {
      ...
      if (field.options && field.options.length > 0) {
        const optionItems = field.options.map((opt, i) =>
          `                                <option${i}
                                    jcr:primaryType="nt:unstructured"
                                    text="${opt.text}"
                                    value="${opt.value}"/>`
...
export function dialogToJson(dialog: ComponentDialog): Record<string, unknown> {
  ...
  for (const tab of dialog.tabs) {
    ...
    for (const field of tab.fields) {
      ...
      if (field.options) {
        fieldContent['items'] = field.options.reduce((acc, opt, i) => {
          acc[`option${i}`] = {
            'jcr:primaryType': 'nt:unstructured',
            'text': opt.text,
            'value': opt.value,
