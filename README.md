...
└── extension/                # App Builder Universal Editor extension
    ├── app.config.yaml       # Top-level App Builder config ($include of ext.config)
    ├── eslint.config.js      # Flat ESLint config (@adobe/eslint-config-aio-lib-config)
    └── src/
        ├── app-metadata.json # Extension registration metadata (generated by hook)
        └── universal-editor-ui-1/
            ├── ext.config.yaml  # Extension point universal-editor/ui/1
            ├── web-src/         # Parcel web bundle (index.html + panel.html)
            └── actions/         # Runtime actions (generate, suggest)
```

### Universal Editor Extension

The `extension/` directory is an [Adobe App Builder](https://developer.adobe.com/app-builder/docs/overview/)
extension that surfaces the AEM A2UI assistant inside the AEM Universal Editor rail.

- **Extension point:** `universal-editor/ui/1` (see `src/universal-editor-ui-1/ext.config.yaml`)
- **`web-src/index.html`** registers the extension via `@adobe/uix-guest` and posts events over a `BroadcastChannel`
- **`web-src/panel.html`** is the rail host: it `attach()`es to the host page and iframes the Java agent's `/extension-panel` endpoint
- **`actions/`** are OpenWhisk runtime actions (`generate`, `suggest`) that call the Java agent at `AI_AGENT_URL`
...
cd extension
npm install
npm run build          # aio app build (2 actions + Parcel web bundle)
npm run lint           # eslint src/
aio app dev            # needs .env with IMS credentials (see .env template)
```

The built web bundle lands in `dist/universal-editor-ui-1/web-prod/` and can be served
statically for local testing (e.g. `python3 -m http.server`). The registration page
(`index.html`) and rail host (`panel.html`) load independently of the App Builder host;
`panel.html` shows an error state if no host page is connected.

> **Note:** The extension has not been deployed yet. Deployment requires `aio login` with
> Adobe Developer Console credentials (`aio app deploy`).
...
- `@adobe/aio-cli` (for the App Builder extension) — `npm install -g @adobe/aio-cli`
